...
public class AuthenticationController {
    ...
    public ResponseEntity<?> register(@RequestBody @Valid RegisterDTO registerData) {
        if (userRepository.findByUsername(registerData.username()).isPresent()) return ResponseEntity.badRequest().body("Username already exists");
        String encodedPassword = new BCryptPasswordEncoder().encode(registerData.password());
        User newUser = new User(registerData.username(), encodedPassword, UserRole.USER);
